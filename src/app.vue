...
const font = $computed(() => `900 ${fontSize}px Noto Serif SC`)
...
function putText (ctx, ch, origin, x, y) {
  ...
  if (import.meta.env.DEV && DEBUG.showCharArea) {
    ...
    ctx.fillRect(rx, ry, fontSize, fontSize)
  }
  ctx.restore()
  ...
  const fx = rx + left - width / 2 + fontSize / 2
  const fy = ry + top - height / 2 + fontSize / 2
  ...
  if (import.meta.env.DEV && DEBUG.showCharArea) {
    ...
    ctx.fillStyle = '#00f8'
    // ctx.fill()
    ctx.fillRect(fx - left - top, fy + right + bottom, fontSize, fontSize)
    ctx.restore()
  }
}
</script>
