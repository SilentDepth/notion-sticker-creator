...
export default function ({ offset = 17 / 238, style = '' }: Props, debug?: boolean) {
  const date = new Date()
  ...
  return h(
    ...
    [
      ...
      h(
        'span',
        { style: `margin: 7px 0; font-size: 150px; height: 1em; color: crimson; transform: translateY(-${150 * offset}px); ${debug ? 'box-shadow: 0 0 0 1px #f0f' : ''}` },
        String(date.getDate()),
      ),
      h(
        'span',
